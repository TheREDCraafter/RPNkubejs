            ...
            .then(Commands.literal("ring")
                .executes(context => {
                    ...
                    context.source.player.getServer().players.forEach(element => {
                        ...
                        element.runCommandSilent(`tellraw @s[tag=rpn.ring] {"text":"Im Rathaus wurde geklingelt!","color":"green","bold":true,"hoverEvent":{"action":"show_text","value":[{"text":"${context.source.player.getName()}"}]}}`);
                    });
                    return 1;
                })
            )
