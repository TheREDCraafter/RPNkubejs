            ...
            .then(Commands.argument("stamp", Arguments.STRING.create(event))
                .then(Commands.argument("name", Arguments.STRING.create(event))
                    .executes(context => {
                        ...
                        console.log(`give @s minecraft:written_book{title: "[${stamp}] ${name}", author: "Rathaus [IC]", display: {Lore:['{"text":"[${stamp}] | Infinity City","color":"dark_purple","italic":false}']}, pages: ${pages.toString().replace("\n", "\\n")}}`)
